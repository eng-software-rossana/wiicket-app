...
const scrollViewStyle = StyleSheet.create({
  ...
  headerScroll: { height: 40, maxHeight: 40 },
...
const json_data = [
  {
    productID: '99999',
    title: 'Imagem do monte seu pc',
    cost: 0.0,
    description: 'Imagem do monte seu pc',
    category: 'IMG',
    pcBuildImg: true,
  },
  {
  ...
  },
  {
    ...
    description: 'Placa de Vídeo ASUS AMD Radeon RX 6700 XT - Descrição Padrão',
    category: 'Placa de vídeo',
  },
  {
    productID: '11',
  ...
  {
    productID: '18',
    title: 'Memória XPG Spectrix D41 TUF, RGB, 8GB, 3200MHz, DDR4, CL16, Preta',
    cost: 457.99,
    imgURI:
  ...
  {
    productID: '28',
    title: 'Gabinete Gamer Redragon Grapple, Mid Tower, RGB, Lateral em Vidro',
    cost: 299.99,
    imgURI:
...
const categ: Product[] = Object.values(json_data);

const Home = () => {
  const [searchText, setSearchText] = useState('');
  const [products, setProducts] = useState<Product[]>(categ);
    ...
    <TouchableOpacity
      onPress={() => {
        if (category.name === 'Todos') {
          ...
          setProducts(
            categ.filter(product => product.category === category.name),
          );
        }
      }}
      ...
      <View>
        <Appbar.Header style={headerBarStyles.headerColor}>
          <TextInput
            ...
            onSubmitEditing={() => {
              const content = categ.filter(product =>
